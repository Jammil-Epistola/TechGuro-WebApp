...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='🌱 Seed TechGuru database with all content',
...
{Colors.BOLD}Note:{Colors.ENDC} This script calls the individual seeders in TGbackend/seeders/ folder
    ...
    try:
        print_header("🌱 TECHGURU DATABASE SEEDER 🌱")
        ...
        print_info("Your TechGuru database is ready to use!")
...
def seed_courses():
    ...
    try:
        # Import and run the course seeder main function
        from TGbackend.seeders.seedCourse import main as seed_course_main
        
        print_info("Running seedCourse.py...")
        seed_course_main()
        print_success("Courses and assessments seeded successfully!")
...
def seed_milestones():
    ...
    try:
        # Import and run the milestone seeder main function
        from TGbackend.seeders.seedMilestone import main as seed_milestone_main
        
        print_info("Running seedMilestone.py...")
        seed_milestone_main()
        print_success("Milestones seeded successfully!")
...
def seed_quizzes():
    ...
    try:
        # Import and run the quiz seeder
        from TGbackend.seeders.seedQuiz import seed_quizzes
        
        print_info("Running seedQuiz.py...")
        seed_quizzes()
        print_success("Quizzes seeded successfully!")
...
def print_database_summary():
    ...
    try:
        ...
        # Course Summary
        courses = db.query(models.Course).all()
        print(f"{Colors.BOLD}📚 Courses: {len(courses)}{Colors.ENDC}")
        ...
        # Milestone Summary
        milestones = db.query(models.Milestone).all()
        print(f"\n{Colors.BOLD}🏆 Milestones: {len(milestones)}{Colors.ENDC}")
...
def main():
    ...
    # Parse command line arguments
    parser = argparse.ArgumentParser(
        description='Seed TechGuru database with all data',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python -m TGbackend.seedAll                 # Seed everything
  ...
  python -m TGbackend.seedAll --quizzes       # Only seed quizzes
    ...
    try:
        print_header("TECHGURU DATABASE SEEDER")
        print_info(f"Starting database seeding process...")
